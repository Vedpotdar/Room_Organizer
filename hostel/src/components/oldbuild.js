...
function HostelRoomOLD() {
  ...
  const partners = async () => {
    if (capacity == currentCap) {
      alert("No Students in this Room");
    } else {
      ...
      for (let i = 0; i < decideNo; i++) {
        ...
        try {
          const result = await axios.post("http://localhost:3003/allStdone", {
            StduntPath,
          });
  ...
  useEffect(() => {
    ...
    function generateRooms(floorSelector, start, end) {
      ...
      for (let i = start; i <= end; i++) {
        ...
        axios
        .post("http://localhost:3003/Allcapacity", { i })
        .then((result) => {
          console.log(result.data);
          ...
          console.log("ALL Capacity:", capacity);
          console.log("ALL Room Number:", roomNumber);
          console.log("ALL CurrentCapacity:", currentCap);
          if(currentCap==0){
            roomDiv.classList.add("full");
          }
          else if(currentCap==capacity){
            roomDiv.classList.add("empty");
          }
          else{
            roomDiv.classList.add("partial");
          }
        ...
        roomDiv.addEventListener("click", function () {
          ...
          console.log(i);
          axios
            .post("http://localhost:3003/roomdetails", { i })
            .then((result) => {
              console.log(result.data);
              const roomDetails = result.data[0]; // Access the first object in the array
              const capacity = roomDetails.capacity; // Access the capacity property
    ...
    generateRooms(
      ".book-selector:nth-child(1)",
      floors["Ground Floor"].start,
      floors["Ground Floor"].end
    );
    generateRooms(
      ".book-selector:nth-child(2)",
      floors["First Floor"].start,
      floors["First Floor"].end
    );
    generateRooms(
      ".book-selector:nth-child(3)",
      floors["Second Floor"].start,
      floors["Second Floor"].end
    );
      ...
      <Container>
                ...
                <div className="rooms">
                  <div className="status">
                    <div className="item empty">
                      <span className="status-color"></span>Empty
                    </div>
                    <div className="item full">
                      <span className="status-color"></span>Full
                    </div>
                    <div className="item partial">
                      <span className="status-color"></span>Partial
                    </div>
                    <div className="item selected">
                      <span className="status-color"></span>Selected
                    </div>
                  </div>
        ...
        >
          <Modal.Dialog id="roomDetailBox">
            ...
            <Modal.Footer>
              <Button variant="secondary" onClick={partners}>
                Show Room Partner Details
              </Button>
